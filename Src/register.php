...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($error)) {
        ...
        if ($stmt = $pdo->prepare($sql)) {
            ...
            $error = "Oops! Une erreur est survenue. Veuillez réessayer.";
        }
    }

    if (empty($error)) {
        $password_hash = password_hash($password, PASSWORD_DEFAULT);
        $sql = "INSERT INTO users (fullname, email, password_hash, phone) VALUES (:fullname, :email, :password_hash, :phone)";
        if ($stmt = $pdo->prepare($sql)) {
            $stmt->bindValue(":fullname", $fullname, PDO::PARAM_STR);
            $stmt->bindValue(":email", $email, PDO::PARAM_STR);
            $stmt->bindValue(":password_hash", $password_hash, PDO::PARAM_STR);
            $stmt->bindValue(":phone", !empty($phone) ? $phone : null, PDO::PARAM_STR);
            if ($stmt->execute()) {
...
<body>
    <main class="wrap" role="main">
        <section class="promo" aria-hidden="false">
            <div class="brand">
                ...
                <div>
                    <h1>Guardia — Création de compte</h1>
                    <div style="font-size:13px;color:var(--muted)">Sécurisé • Professionnel • Accessible</div>
                </div>
            </div>
            <p class="lead">Rejoignez Guardia et gérez vos projets en toute sécurité.</p>
        </section>

        <section class="card" aria-labelledby="signup-title">
            <h2 id="signup-title" style="margin:0 0 6px 0">Créer un compte</h2>
            <p style="margin:0 0 14px 0;color:var(--muted)">Remplissez les informations ci-dessous pour commencer.</p>

            <!-- Affichage erreur / succès serveur -->
            <?php if (!empty($error)): ?>
            ...
            <form id="registerForm" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" novalidate>
                <div class="form-row">
                    <div>
                        ...
                        <input id="fullname" name="fullname" type="text" autocomplete="name" required minlength="2" placeholder="Prénom Nom" value="<?php echo htmlspecialchars($fullname); ?>" />
                        <div class="errors" id="err-fullname" aria-live="polite"></div>
                    </div>
                    <div>
                        <label for="phone">Téléphone (optionnel)</label>
                        <input id="phone" name="phone" type="tel" inputmode="tel" pattern="^[0-9+()\-.\s]{6,25}$" placeholder="+33 6 12 34 56 78" value="<?php echo htmlspecialchars($phone); ?>" />
                        <div class="field-help">Format international accepté</div>
                ...
                <div>
                    ...
                    <input id="email" name="email" type="email" autocomplete="email" required placeholder="vous@exemple.com" value="<?php echo htmlspecialchars($email); ?>"/>
                ...
                <div class="form-row">
                    <div>
                        ...
                        <div class="password-wrap">
                            ...
                            <button type="button" class="toggle-eye" id="togglePwd" aria-label="Afficher le mot de passe">👁️</button>
                        </div>
                        <div class="strength" aria-hidden="true" title="Indicateur de force">
                            <i id="pwdStrengthBar"></i>
                        </div>
                        <div class="field-help">Utilisez lettres, chiffres et caractères spéciaux.</div>
                ...
                <div class="controls">
                    ...
                    <div style="font-size:13px;color:var(--muted)">Déjà inscrit ? <a href="./login.php" style="color:var(--accent);text-decoration:none">Se connecter</a></div>
                </div>
    ...
    <script>
        // Validation client légère — soumet le formulaire au serveur si OK
        ...
        form.addEventListener('submit', (e) => {
            ...
            let ok = true;
            if(!fullname.value || fullname.value.trim().length < 2){ setError('fullname','Veuillez saisir votre nom complet (2 caractères minimum).'); ok=false; }
            if(!email.value || !emailValid(email.value.trim())){ setError('email','Adresse e-mail invalide.'); ok=false; }
            if(!pwd.value || pwd.value.length < 8){ setError('password','Le mot de passe doit contenir au moins 8 caractères.'); ok=false; }
            if(pwd.value !== confirm.value){ setError('confirm','Les mots de passe ne correspondent pas.'); ok=false; }
            if(!phoneValid(phone.value.trim())){ setError('phone','Numéro de téléphone invalide.'); ok=false; }
